...
func newDeployProgressTracker(writer io.Writer, interactive bool, serviceNames []string) *deployProgressTracker {
...
func (t *deployProgressTracker) Update(serviceName string, phase deployPhase, detail string) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *deployProgressTracker) Render() {
	...
	// Header
	header := fmt.Sprintf("  %-*s  %-12s  %-8s  %s", maxName, "Service", "Phase", "Elapsed", "Detail")
	...
	for _, svc := range t.services {
		...
		line := fmt.Sprintf("  %s %-*s  %-12s  %-8s  %s",
			icon, maxName, svc.name, svc.phase, elapsed, detail)
...
func (t *deployProgressTracker) RenderFinal() {
	...
	if t.interactive && t.lastLines > 0 {
		buf.WriteString(fmt.Sprintf("\033[%dA", t.lastLines))
	...
	buf.WriteString(fmt.Sprintf("\n  %-*s  %-12s  %s\n", maxName, "Service", "Status", "Duration"))
	...
	for _, svc := range t.services {
		...
		buf.WriteString(fmt.Sprintf("  %s %-*s  %-12s  %s\n",
			icon, maxName, svc.name, svc.phase, elapsed))
	}
...
// StartTicker begins periodic rendering and returns a cancel function.
func (t *deployProgressTracker) StartTicker(ctx context.Context) context.CancelFunc {
	if !t.interactive {
