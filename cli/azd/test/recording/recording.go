...
func Start(t *testing.T, opts ...Options) *Session {
	...
	vcr.SetMatcher(func(r *http.Request, i cassette.Request) bool {
		...
		if strings.Contains(r.URL.Path, roleAssignmentsPath) {
			// roleAssignments are autogenerated by azd each time as a GUID.
			recorded, err := url.Parse(i.URL)
			if err != nil {
				panic(err)
			}
			replaceWith := roleAssignmentsPath + "IGNORED?api"
			pathIgnoredId := roleAssignmentsRegex.ReplaceAllString(r.URL.String(), replaceWith)
			recordedIgnoredId := roleAssignmentsRegex.ReplaceAllString(recorded.String(), replaceWith)
			log.Info("recorderProxy: ignoring roleAssignments ID name in role assignments request", "url", r.URL)
			return r.Method == i.Method && pathIgnoredId == recordedIgnoredId
		}
