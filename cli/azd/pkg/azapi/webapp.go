	...
	if isLinux {
		...
		for attempt := range maxBuildRetries + 1 {
			...
			if attempt > 0 {
				...
				if waitErr := waitForScmReady(ctx, client, progressLog); waitErr != nil {
...
func waitForScmReady(ctx context.Context, client scmReadyChecker, progressLog func(string)) error {
	const (
		scmReadyTimeout = 90 * time.Second
