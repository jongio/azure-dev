...
func TestSSRFSafeRedirect_BlocksPrivateHostnames(t *testing.T) {
	t.Parallel()
	// A hostname redirect where the DNS lookup itself resolves to a private IP
	// is not testable without a mock DNS resolver. This test verifies that
	// well-known localhost names are blocked by the redirect handler (since
	// net.LookupHost("localhost") returns 127.0.0.1 on most systems).
	tests := []struct {
		host    string
		blocked bool
	}{
