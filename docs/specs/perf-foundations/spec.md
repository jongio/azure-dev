...
to absolute paths relative to the service directory. This centralizes path resolution that was
previously duplicated across `Build()`, `runRemoteBuild()`, and other call sites, while
preserving the existing service-relative resolution semantics.

Called from `Build()` and `runRemoteBuild()`. The same logic is mirrored in
`useDotNetPublishForDockerBuild()` for consistency. `packBuild()` was also updated to resolve
user-specified `docker.context` relative to the project root instead of the service path. Tests
updated to expect absolute resolved paths, including dynamic resolution in the table-driven
`Test_DockerProject_Build` test.
